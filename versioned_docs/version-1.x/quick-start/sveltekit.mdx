...
- The [`@zenstackhq/server/sveltekit`](../reference/server-adapters/sveltekit) server adapter provides a SvelteKit hooks handler that installs an automatic CRUD API for database operations.

- The [`@zenstackhq/tanstack-query`](../reference/plugins/tanstack-query) plugin provides automatic generation of client-side query and mutation hooks for consuming the afore-mentioned CRUD API.
...
[here](https://github.com/zenstackhq/docs-tutorial-sveltekit/tree/v1). For more details about ZenStack,
please refer to the [Reference](../category/reference) and [Recipes](../category/recipes)
