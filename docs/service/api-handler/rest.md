...
- `nestedRoutes`
  ...
  Optional. A `boolean` field that controls whether to enable endpoints not part of the JSON:API spec for accessing nested related resources. Enabling it will allow:

  - [🔗](#fetching-a-specific-related-resource) Reading a specific "to-many" related resource scoped to a parent resource. 
  - [🔗](#creating-a-related-resource) Creating a nested resource under a parent resource.
  - [🔗](#updating-a-related-resource) Updating a nested resource under a parent resource.
...
### Fetching a specific related resource

:::info
This endpoint is not part of the JSON:API spec. Only available when the `nestedRoutes` option is enabled in `RestApiHandler` configuration.
:::

A specific related resource from a "to-many" relation can be fetched using the following endpoint:

```ts
GET /:type/:id/:relationship/:nestedId
```
...
A new resource can be created under a parent resource using the following endpoint:

```ts
POST /:type/:id/:relationship
```
...
- 400: The request was malformed.
- 403: The request was forbidden.
- 404: The specified parent resource does not exist.
...
### Updating a related resource

:::info
This endpoint is not part of the JSON:API spec. Only available when the `nestedRoutes` option is enabled in `RestApiHandler` configuration.
:::

A related resource can be updated under a parent resource using the following endpoint:

```ts
// for "to-many" relationship
PUT /:type/:id/:relationship/:nestedId
PATCH /:type/:id/:relationship/:nestedId

// for "to-one" relationship
PUT /:type/:id/:relationship
PATCH /:type/:id/:relationship
```
...
- 400: The request was malformed.
- 403: The request was forbidden.
- 404: The specified parent resource or the related resource does not exist.
