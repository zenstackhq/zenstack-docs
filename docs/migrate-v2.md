...
Two most important packages have been renamed:
...
V2 had the concept of [Abstract Model](/docs/2.x/guides/multiple-schema#abstract-model-inheritance) that allows you to define base models that serve purely as base types, but are not mapped to the database. You can use the `extends` keyword to inherit from an abstract model. We found this to be confusing because `extends` is also used for inheriting from a [Polymorphic Model](/docs/2.x/guides/polymorphism). The same keyword was used for two very different concepts.
...
The [check()](/docs/2.x/guides/check-permission) feature is removed due to low popularity.
...
The [NestJS](https://nestjs.com/) adapter is removed because it was a very shallow encapsulation layer and didn't add much value. Instead, a new [NestJS guide](./recipe/nestjs) is added.
