...
Let's run the `zenstack` CLI to prepare your project for using ZenStack. The CLI will also install Prisma dependencies.
...
Then, update the content of `auth/server/utils/db.ts` to the following:
...
Next, run `zenstack` CLI to regenerate Prisma schema, and sync it to the database:
...
All CRUD calls made via the enhanced client will be subject to the access policies. Since the ORM is already secured, ZenStack can automatically mount a set of CRUD APIs into your Nuxt app. To achieve it, first install an extra server adapter package:
...
We can write code to consume the CRUD APIs, but that would be quite boring. Instead, we can use a plugin to generate them for us. First, install the `@zenstackhq/tanstack-query` plugin package:
...
Then, create a reuseable `Post` component under `components/Post.vue`:
...
If you have trouble following the building process, you can find the final result [here](https://github.com/zenstackhq/docs-tutorial-nuxt). For more details about ZenStack, please refer to the [Reference](/docs/category/reference) and [Guides](/docs/category/guides) parts of the documentation.
