...
ZenStack gives you a pleasant type-safe approach for calling to your database "directly" from the frontend without coding a backend.
However, even if you're only building the backend side of an app, it can still help by defining access policies right inside your data model.
This results in a security model that's both easier to maintain and more reliable than a manually implemented one.
...
1. Published post can be be viewed by all users.
...
npx degit prisma/prisma-examples/blob/latest/typescript/rest-express my-blog-app
cd my-blog-app
...
, and verify it works by:

```bash
curl localhost:3000/post
```

You should see a list of posts like:
...
However they don't carry any access policies with them, meaning that the service is wide
open: anyone can do anything to any data.
...
### 4. Secure up the APIs

Now let's secure up our APIs leverating the access policies.
...
function getPrisma(req: Request) {
    return withPolicy(prisma, {
        user: { id: parseInt(req.header('X-USER-ID')!) },
    });
}
````
...
Then, change our `POST /post` API to use extracted user id instead of passing in user email in body when creating a new post:
...
Now, if you hit the `/post` endpoint again directly, you should get a 403 error:
...
In case you met any trouble following the building process, you can find the final result [here](https://github.com/zenstackhq/docs-tutorial-express). For more details about ZenStack, please refer to the [Reference](/docs/category/reference) and [Guides](/docs/category/guides) parts of the documentation. Have fun building cool stuff 🚀!
