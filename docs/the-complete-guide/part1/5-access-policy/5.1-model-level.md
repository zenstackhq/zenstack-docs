...
The trick is to consider which side of the relation will have a foreign key update. In this example, the foreign key `authorId` of `Post#1` will be updated, so `Post#1` needs to satisfy its "update" policies.
