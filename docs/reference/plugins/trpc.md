...
The `@zenstack/trpc` plugin generates a [tRPC router](https://trpc.io/docs/router) for making CRUD calls to the database. You can use it directly as your main router, or merge it with other routers to create a more complex setup. The router syntactically mirror the APIs of a standard Prisma client, including the function names and shapes of parameters (hooks directly use types generated by Prisma).
