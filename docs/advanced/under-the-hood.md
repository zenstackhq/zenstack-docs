...
-   Policy: `@core/policy`
    ...
    -   `user`: the current user, which serves as the return value of [`auth()`](/docs/the-complete-guide/part1/access-policy/#accessing-user-data) in the policy rules.
    -   `preValue`: the previous value of an entity before update (for supporting the [`future()`](/docs/the-complete-guide/part1/access-policy/#update) function in the policy rules).
