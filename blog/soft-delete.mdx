...
You can see this one has been brought up a lot. I think it is from the database point of view to main referential integrity. From the practical point of view, the benefit is still for recovery.
...
You might think it doesn’t seem to be hard to fix it for both by adding the deleted filter. It might be for the above example, but how about this one:
...
Conversely, we had fewer real-world use cases. So if you guys meet any case that we missed, we would appreciate it if you could create an issue in [Github](https://github.com/zenstackhq/zenstack/issues) or directly throw it to our [Discord](https://go.zenstack.dev/chat). Let’s make ZenStack better together!
