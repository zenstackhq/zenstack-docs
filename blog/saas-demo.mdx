...
In addition to the Prisma schema, it has two attributes `@length` and `@regex`, which is the standard validation attribute provided by ZenStack library. Actually, one enhancement to Prisma of ZenStack is to be able to define your own [custom attribute](https://zenstack.dev/docs/2.x/reference/zmodel-language#attribute).
...
-   `User`
    ...
    It uses the Any condition syntax in [Collection Predicate Expression](https://zenstack.dev/docs/2.x/reference/zmodel-language#collection-predicate-expressions) :
...
First install the `@zenstackhq/next`and `@zenstackhq/swr`packages:
