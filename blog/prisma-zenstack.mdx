---
description: Modern Web Architecture Without a Backend — Using Prisma + ZenStack
...
While at a separate level two trends are going on:

-   **Consolidation of architecture**

    Developers are getting really tired with combining so many different pieces in an application, and have started to favor less for more: unified programming language, mono-repos, meta frameworks (like Next.js), and even building web apps without expliclty coding a backend.

-   **Renaissance of SQL databases**

    In reality SQL databases probably never really lost any ground to NoSQL; it just became less cool to use for a while, from a fashion perspective. Now with more and more people aware that few web apps are indeed "web scale" and relational databases are still the best choice for most use cases, SQL databases are making a comeback.
...
[ZenStack](https://zenstack.dev) is an extension to Prisma that adds a layer of security to it. It allows you to define access control rules inside your schema file, and inject them into Prisma queries at runtime.

![ZModel](/img/banner-code.png)

Furthermore, it provides a RESTful API to access the database, and also generates client libraries for your frontend code. By combining access control and API generation, ZenStack makes it possible to have a fully secure CRUD backend without manually implementing it.
...
Use the **ZModel** language (a superset of Prisma schema) to define your models, relations, and access policies. Optionally, if you use Next.js as your full-stack framework, enable the `@zenstackhq/react` plugin to generate data access client hooks.
...
Use the generated hooks to build up UI parts with ease. The hooks talk to the data access API installed in the previous step. Since the API is already secured by the access policies, the hooks will only return entities that are readable to the current user, and will reject any unauthorized mutations.
...
### 1. Clear picture of data model

By using a DSL to model your entities, you always have an unambiguous description of your app's data model centralized in a single easy-to-read file.
...
-   You application has non-trivial security requirements.
...
-   You don't want to be locked in to a specific database kind or hoster.

# Wrap Up

Prisma and ZenStack make a great combination for building up a secure backend with a relational database. We hope you'll find it useful for your next project.
