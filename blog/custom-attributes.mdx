---
description: How to add custom attributes in Prisma
...
2. Custom attributes by ZenStack.
...
All you need to do is to implement the `generatorHandler` function to create a new generator of yourself like the below:
...
**So if the perfect solution doesn’t exist, then I will build it.**

# Attributes in ZenStack
...
Based on the principle of “**never reinvent the wheel**”, we do put a value on the existing generators that contributed by the community, like the zod-prisma and [\*\*prisma-nestjs-graphql](https://github.com/unlight/prisma-nestjs-graphql).\*\* So to make it still work, for any non-Prisma attribute, we will generate it as the triple comments in the `schema.prisma` file.
